...
export default function App() {
  ...
  useEffect(() => {
    const handlePaste = async (e: ClipboardEvent) => {
      ...
      reader.onload = (ev) => {
        ...
        ref.current?.setBackground(dataUrl);
        dispatch({ type: "SET_CANVAS_IMAGE", variant: activeEditVariant, imageUrl: dataUrl, prompt: "Pasted from clipboard" });
      };
  ...
  const handleClearWorkspace = useCallback(() => {
    if (!confirm("Clear all canvas content and start a fresh project?")) return;
  ...
  const handleUseAsset = useCallback((asset: AssetItem) => {
    getActiveRef(activeEditVariant).current?.addImage(asset.base64, asset.role);
  }, [activeEditVariant]);
  ...
  const handleAddToQueue = useCallback(async (variant: CanvasVariant) => {
    ...
    try {
      ...
      const slug = canvas.seoData.filenameSlug || `thumbnail-variant-${variant.toLowerCase()}`;
      const item: ExportQueueItem = { id: `queue_${Date.now()}`, variant, imageBase64, filename: `${slug}.png`, addedAt: new Date() };
  ...
  const handleFlattenDownloadBoth = useCallback(async () => {
    for (const v of (state.showVariantB ? ["A", "B"] : ["A"]) as CanvasVariant[]) {
    ...
    <StoreContext.Provider value={{ state, dispatch }}>
      <div className="flex flex-col h-screen overflow-hidden" style={{ background: "var(--bg-main)" }}>
        ...
        <header style={{
          ...
          justifyContent: "space-between", flexShrink: 0, zIndex: 100, gap: 8,
        }}>
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              title="Clear workspace and start fresh"
            >
              <Trash2 size={12} />
            ...
            <button className="btn-secondary px-2 py-1.5 text-xs flex items-center gap-1.5" onClick={() => setSettingsOpen(!settingsOpen)}>
              <Settings size={12} />
              <span className="hidden sm:inline">{settingsOpen ? "Close" : "Branding"}</span>
            </button>
            <button className="btn-primary px-2 sm:px-3 py-1.5 text-xs sm:text-sm flex items-center gap-1.5" onClick={handleFlattenDownloadBoth} style={{ fontWeight: 700 }}>
              <Download size={14} />
        ...
        <div className="flex flex-1 overflow-hidden flex-col md:flex-row">
          ...
          {/* Desktop Sidebar */}
          <aside className="hidden md:flex" style={{ width: 300, background: "var(--bg-card)", borderRight: "1px solid var(--border-color)", flexDirection: "column", flexShrink: 0, overflow: "hidden" }}>
            {sidebarContent}
          </aside>

          {/* Mobile Panel */}
          <div className="flex md:hidden flex-col" style={{ background: "var(--bg-card)", borderBottom: "1px solid var(--border-color)", flexShrink: 0, overflow: "hidden", maxHeight: mobilePanelOpen ? "65vh" : 0, transition: "max-height 0.3s ease" }}>
          ...
          <button
            ...
            style={{ background: "var(--bg-card)", borderBottom: "1px solid var(--border-color)", color: "var(--accent-cyan)", flexShrink: 0, letterSpacing: "0.05em" }}
            onClick={() => setMobilePanelOpen(o => !o)}
          >
            {mobilePanelOpen ? <><ChevronUp size={14} /> Hide Controls</> : <><ChevronDown size={14} /> Show AI Controls</>}
          </button>

          {/* Main Canvas Area */}
          <main className="flex-1 overflow-y-auto overflow-x-hidden" style={{ background: "var(--bg-main)", padding: 12 }}>
            <div className={`grid gap-4 ${state.showVariantB ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-1"}`} style={{ maxWidth: state.showVariantB ? "none" : 800, margin: "0 auto" }}>
                  ...
                  <div key={variant} className="flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <div style={{ width: 28, height: 28, borderRadius: 6, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 14, background: isActive ? "var(--gradient)" : "var(--bg-input)", color: isActive ? "#0a0e27" : "var(--text-muted)" }}>
                          {variant}
                        </div>
                        <span className="variant-label" style={{ color: isActive ? "var(--accent-cyan)" : "var(--text-muted)" }}>Variant {variant}</span>
                        {canvas.backgroundImage && (
                          <span style={{ fontSize: 9, color: "var(--accent-green)", background: "rgba(0,255,136,0.1)", padding: "1px 6px", borderRadius: 10, border: "1px solid rgba(0,255,136,0.3)" }}>● LIVE</span>
                        )}
                      </div>
                    ...
                    <div style={{ position: "relative" }}>
                      <CanvasPanel
                        ...
                        onActivate={() => { dispatch({ type: "SET_ACTIVE_VARIANT", variant }); setActiveEditVariant(variant); }}
                    ...
                    {isActive && (
                      <EditingToolbar
                        variant={variant}
                        onAddText={(text, style, color) => ref.current?.addText(text, style, color)}
                        onAddArrow={(color) => ref.current?.addArrow(color)}
            ...
            {/* Bottom export bar */}
            <div style={{ marginTop: 20, padding: "12px 16px", background: "var(--bg-card)", border: "1px solid var(--border-color)", borderRadius: 12, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              <div>
                <p style={{ fontWeight: 700, fontSize: 14 }}>Flatten & Export</p>
                <p style={{ fontSize: 11, color: "var(--text-muted)" }}>Exports at {nW}×{nH}px — optimized for YouTube upload</p>
              </div>
              <div className="flex gap-2 flex-wrap">
                ...
                <button className="btn-primary px-4 py-2 text-sm flex items-center gap-2" style={{ fontWeight: 700 }} onClick={handleFlattenDownloadBoth}>
                  <Download size={16} /> Flatten & Download {state.showVariantB ? "Both PNGs" : "PNG"}
                </button>
              </div>
            </div>
          </main>
